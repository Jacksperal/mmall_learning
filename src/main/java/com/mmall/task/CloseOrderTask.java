...
public class CloseOrderTask {
    ...
    public void closeOrderTaskV3() {
        ...
        if (setnxResult != null && setnxResult == 1) {
            ...
            String lockVauleStr = RedisShardedPoolUtil.get(Const.REDIS_LOCK.CLOSE_ORDER_TASK_LOCK);
            if (lockVauleStr != null && System.currentTimeMillis() > Long.parseLong(lockVauleStr)) {
                String getSetResult = RedisShardedPoolUtil.getSet(Const.REDIS_LOCK.CLOSE_ORDER_TASK_LOCK, String.valueOf(System.currentTimeMillis() + lockTimeout));
                //再次用当前时间戳getset
                //返回给定的key的旧值,->旧值判断,是否可以获取锁
                //当key没有旧值时,即key不存在时,返回nil -> 获取锁
                //这里我们set了一个新的value值,获取旧的值
                if (getSetResult == null || (getSetResult != null && StringUtils.equals(lockVauleStr, getSetResult))) {
                    //真正获取到锁
