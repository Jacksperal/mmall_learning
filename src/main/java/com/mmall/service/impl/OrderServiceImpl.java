...
public class OrderServiceImpl implements IOrderService {
    ...
    private OrderVo assembleOrderVo(Order order, List<OrderItem> orderItemList) {
        ...
        orderVo.setImageHost(MyPropertiesUtil.getProperty("ftp.server.http.prefix"));

        List<OrderItemVo> orderItemVoList = Lists.newArrayList();
        for (OrderItem orderItem : orderItemList) {
            OrderItemVo orderItemVo = this.assemebleOrderItemVo(orderItem);
    ...
    public ServerResponse<String> cancel(Integer userId, Long orderNo) {
        ...
        if (order.getStatus() != Const.OrderStatusEnum.NO_PAY.getCode()) {
            return ServerResponse.createByErrorMessage("已经付款，无法取消该订单");
        }
    ...
    public ServerResponse getOrderCartProduct(Integer userId) {
        ...
        List<OrderItemVo> orderItemVoList = Lists.newArrayList();
        for (OrderItem orderItem : orderItemList) {
            orderItemVoList.add(this.assemebleOrderItemVo(orderItem));
        }
        orderProductVo.setOrderItemVoList(orderItemVoList);
        orderProductVo.setImageHost(MyPropertiesUtil.getProperty("ftp.server.http.prefix"));
    ...
    public ServerResponse pay(Long orderNo, Integer userId, String path) {
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl(MyPropertiesUtil.getProperty("alipay.callback.url"))//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                try {
                    isSuccess = FTPUtil.uploadFile(fileList);
                } catch (IOException e) {
                    logger.error("上传二维码异常", e);
                }
                if (isSuccess) {
                    String qrUrl = MyPropertiesUtil.getProperty("ftp.server.http.prefix") + targetFile.getName();
    ...
    public ServerResponse aliCallback(Map<String, String> params) {
        ...
        String tradeStatus = params.get("tarde_status");
    ...
    public ServerResponse<OrderVo> manageDetail(Long orderNo) {
        Order order = orderMapper.selectByOrderNo(orderNo);
        if (order == null) {
            ServerResponse.createByErrorMessage("该订单不存在");
        }
    ...
    public ServerResponse<PageInfo> manageSearch(Long orderNo, int pageNum, int pageSize) {
        PageHelper.startPage(pageNum, pageSize);
        String searchOrderNo = new StringBuilder().append("%").append(orderNo.toString()).append("%").toString();
        List<Order> orderList = orderMapper.searchByOrderNo(searchOrderNo);
    ...
    public ServerResponse<String> mangeSendGoods(Long orderNo){
        ...
        return ServerResponse.createByErrorMessage("该订单未付款");
    }
